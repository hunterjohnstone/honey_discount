...
const DiscoverClient = ({ initialData }: DiscoverClientProps) => {
  const [promotions, setPromotions] = useState<LoosePromotion[]>(initialData.data);
  const [allData, setAllData] = useState<Promotion[]>([]);
  ...
  const [filteredPromotions, setFilteredPromotions] = useState<LoosePromotion[]>(initialData.data);
  const [pagination, setPagination] = useState(initialData.pagination);

  // Fetch data for client-side navigation
  const fetchData = async (page: number = 1) => {
    setLoading(prev => ({ ...prev, page: true }));
    try {
      const response = await fetch(`/api/product/pagination?page=${page}&pageSize=54`);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-3">
      ...
      <main className="container mx-auto py-8 px-4">
        ...
        {isAddingPromotion && user && <AddPromoForm userId={user.id} onSuccess={() => fetchData(currentPage)} />}
